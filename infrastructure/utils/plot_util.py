...
# @param x_label x轴标签
# @param y_label y轴标签
def draw_simple_plot(x_points: [], y_points: [], labels: [],
                    title: str, x_label: str, y_label: str):
    matplotlib.rcParams['font.sans-serif'] = ['SimHei']

    # 只有一根线
    if len(y_points) == 1:
        plt.plot(x_points, y_points[0], color='royalblue', linestyle='-', label=labels[0])
    # 有两根线
    elif len(y_points) == 2:
        plt.plot(x_points, y_points[0], color='orangered', linestyle='-', label=labels[0])
        plt.plot(x_points, y_points[1], color='royalblue', linestyle='-', label=labels[1])
    # 更多的折线
    else:
        for i in range(len(y_points)):
            plt.plot(x_points, y_points[i], linestyle='-', marker='o', label=labels[i])
